...
Impact Reporter is currently deployed as a static public pilot.

The primary deployment target is Cloudflare Workers using OpenNext for
Cloudflare with Workers Assets.
Vercel remains a fallback if Cloudflare setup creates unexpected friction.

## Required gates before public deployment

- The project maintainer approves the deployment target and public URL.
...
- Public hosted live model calls remain disabled unless explicitly approved by
...
1. Connect the repository in the deployment provider.
...
8. Record the approved URL in the project release notes.
...
1. Connect the repository in Cloudflare Workers/Pages builds or deploy from an
...
2. Secret: `CLOUDFLARE_ACCOUNT_ID`
   - Use the Cloudflare account ID for the deployment account.
...
environment configuration and still requires explicit project maintainer
approval after data-boundary review.

## Enabling hosted live AI later

Do not enable hosted live AI until the project maintainer explicitly approves
the data boundary, model, and deployment window.
...
Current deployment uses the checked-in Cloudflare Worker configuration and
